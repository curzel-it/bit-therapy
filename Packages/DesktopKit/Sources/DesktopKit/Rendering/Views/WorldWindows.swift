...
open class WorldWindows: NSObject, NSWindowDelegate {
    ...
    public init(for world: LiveWorld?) {
        self.tag = "WorldWindows-\(WorldWindows.nextId())"
        self.world = world
        super.init()
    ...
    private func startSpawningWindows() {
        childrenCanc = world?.state.$children.sink { children in
            ...
            children
                ...
                .forEach { child in
                    self.showWindow(representing: child, in: world)
                }
        }
    }
    ...
    private func showWindow(representing entity: RenderableEntity, in world: LiveWorld) {
        let window = dequeueWindow(representing: entity, in: world)
        window.show()
        window.makeKey()
    }
    
    private func dequeueWindow(representing entity: RenderableEntity, in world: LiveWorld) -> EntityWindow {
    ...
    open func newWindow(representing entity: RenderableEntity, in world: LiveWorld) -> EntityWindow {

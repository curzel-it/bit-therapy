...
public class AnimatedSprite: DKCapability, ObservableObject {
    ...
    @MainActor
    func updateSprite() {
        guard let renderable = subject as? RenderableEntity else { return }
    ...
    private func setSprite(_ value: NSImage?) {
        (subject as? RenderableEntity)?.sprite = value
    }
}

// MARK: - Image Animator

public class ImageAnimator: TimedContentProvider<NSImage> {

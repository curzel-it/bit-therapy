...
extension PetAnimation {
    ...
    fileprivate static let abduction = PetAnimation(
        id: "abduction",
...
extension Pet {
    ...
    public class UfoAbduction: Capability {
        ...
        public func abduct(_ target: Entity) {
            self.target = target
            pet?.uninstall(BounceOffLateralBounds.self)
            pet?.uninstall(ResumeMovementAfterAnimations.self)

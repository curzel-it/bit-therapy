...
class PetsAssetsProvider(AssetsProvider):
    ...
    def path(self, sprite: str) -> Optional[str]:
        try:
            ...
            matches = [
                asset
                for asset in self._sorted_assets_by_key[key]
    ...
    def all_assets_for_species(self, species) -> List[str]:
        return [
            path
            for path in self._all_assets_paths
            if self._is_path_of_species(path, species)
        ]
    ...
    def reload_assets(self, folders: List[str]):
        ...
        self._all_assets_paths = sorted([path for paths in all_paths for path in paths])
    ...
    def _build_sorted_assets(self, assets: List[_Asset]):
        ...
        Logger.log(self.tag, "Found", len(self._sorted_assets_by_key), "sprites")

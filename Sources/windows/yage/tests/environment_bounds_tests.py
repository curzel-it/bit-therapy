...
class EnvironmentBoundsNoSafeAreaTests(unittest.TestCase):
    def setUp(self):
        self.env = World(name="test", bounds=Rect(x=0, y=50, width=400, height=900))

    def test_top_bound_properly_placed(self):
        bound = next((c for c in self.env.children if c.id == Hotspot.top_bound.value), None)
    ...
    def test_right_bound_properly_placed(self):
        bound = next((c for c in self.env.children if c.id == Hotspot.right_bound.value), None)
        self.assertIsNotNone(bound)
    ...
    def test_bottom_bound_properly_placed(self):
        bound = next((c for c in self.env.children if c.id == Hotspot.bottom_bound.value), None)
        self.assertIsNotNone(bound)
        self.assertEqual(bound.frame.min_x, 0)
        self.assertEqual(bound.frame.max_x, 400)
    ...
    def test_left_bound_properly_placed(self):
        bound = next((c for c in self.env.children if c.id == Hotspot.left_bound.value), None)
        self.assertIsNotNone(bound)
        self.assertEqual(bound.frame.min_x, 0)

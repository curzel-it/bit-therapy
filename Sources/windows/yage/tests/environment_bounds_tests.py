...
class EnvironmentBoundsNoSafeAreaTests(unittest.TestCase):
    ...
    def test_top_bound_properly_placed(self):
        bound = next(
            (c for c in self.env.children if c.id == Hotspot.TOP_BOUND.value), None
        )
    ...
    def test_right_bound_properly_placed(self):
        bound = next(
            (c for c in self.env.children if c.id == Hotspot.RIGHT_BOUND.value), None
        )
        self.assertIsNotNone(bound)
    ...
    def test_bottom_bound_properly_placed(self):
        bound = next(
            (c for c in self.env.children if c.id == Hotspot.BOTTOM_BOUND.value), None
        )
        self.assertIsNotNone(bound)
        self.assertEqual(bound.frame.min_x, 0)
        self.assertEqual(bound.frame.max_x, 400)
    ...
    def test_left_bound_properly_placed(self):
        bound = next(
            (c for c in self.env.children if c.id == Hotspot.LEFT_BOUND.value), None
        )
        self.assertIsNotNone(bound)
        self.assertEqual(bound.frame.min_x, 0)

...
class AnimationFrameTests(unittest.TestCase):
    def setUp(self):
        self.entity = Entity(
            SPECIES_AGENT, "test", Rect(5, 5, 5, 5), World("", Rect(0, 0, 15, 15))
        )

    def test_animation_frame_is_same_as_entity_frame_if_no_custom_size_or_position_are_set(
        self,
    ):
        animation = EntityAnimation(
            "test", None, EntityAnimationPosition.ENTITY_TOP_LEFT, None
        )

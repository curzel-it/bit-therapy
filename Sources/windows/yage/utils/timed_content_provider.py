...
class TimedContentProvider:
    ...
    def next_content(self, time: float):
        if self.frame_time <= 0 or len(self.contents) == 0: return None
        ...
        contents_skipped = int(math.floor(time_since_last_content_change / self.frame_time))
        used_time = contents_skipped * self.frame_time
        self.leftover_time = time_since_last_content_change - used_time
        
        next_index = (self.current_content_index + contents_skipped) % len(self.contents)

...
class RandomPlatformJumper(Capability):    
    ...
    def _find_platform_and_jump(self):
        if not self.is_enabled or not self.subject.is_alive: return 
    ...
    def _prepare_subject_for_travel(self):
        if animations := self.subject.capability(AnimationsScheduler): animations.is_enabled = False
        if bounce := self.subject.capability(BounceOnLateralCollisions): bounce.is_enabled = False
        if movement := self.subject.capability(LinearMovement): movement.is_enabled = True
        try:
    ...
    def _restore_initial_conditions(self):
        if seeker := self.subject.capability(Seeker): seeker.kill()
        if bounce := self.subject.capability(BounceOnLateralCollisions): bounce.is_enabled = True
        if gravity := self.subject.capability(Gravity): gravity.is_enabled = self._was_gravity_enabled
        if animations := self.subject.capability(AnimationsScheduler): animations.is_enabled = True
        if movement := self.subject.capability(LinearMovement): movement.is_enabled = True

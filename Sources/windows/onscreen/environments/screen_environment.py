...
class ScreenEnvironment(World):
    ...
    def random_pet(self) -> Optional[PetEntity]:
        ...
        if len(pets) > 0: return random.choice(pets)
        return None
    ...
    def remove(self, species_to_remove: Species):
        ...
        to_remove = [child for child in self.children if child.species == species_to_remove]
    ...
    def _bind_pets_on_stage(self):
        d = self._settings.selected_species.subscribe(
    ...
    def _remove_unselected_pets(self, new_selected_species: List[str]):
        ...
        to_remove = [child for child in to_remove if child.species.id not in new_selected_species]
        for child in to_remove: 
            child.kill()
            self.children.remove(child)

...
class SleepingPlace(Capability):
    ...
    def _wake_from_sleep_if_needed(self):
        if self.wake_up_date is None: return 
        if self.wake_up_date > datetime.now(): return
    ...
    def _overlapping_entity_that_can_sleep(self, collisions):
        ...
        collisions = sorted(collisions, key=lambda c: c.intersection.area(), reverse=True)
        if collisions and len(collisions) > 0: return collisions[0].other
        return None
    
    def _sleep_animation(self, entity) -> Optional[EntityAnimation]:
        if self._cached_without_sleep.get(entity.id): return None
        try:
    ...
    def _can_sleep(self, collision: Collision) -> bool:
        if collision.intersection.area() < 100: return False

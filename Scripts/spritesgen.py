...
def export_list():
    ...
    species_and_animations = [
        (species["id"], animation_ids(species)) for species in species
    ]
    species_and_animation = [
        (species, animation)
        for species, animations in species_and_animations
        for animation in animations
    ]
    filenames = [
        f"{species}_{animation}.aseprite"
        for species, animation in species_and_animation
    ]
...
def export_aseprite(file_path, destination_folder):
    ...
    os.system(
        f"{ASEPRITE_BIN} -b {file_path} {IGNORE_LAYERS} --save-as {destination_folder}/{asset_name}-0.png"
    )
...
def exportable_file_paths():
    ...
    return [
        path
        for path in paths
        if any(path.endswith(filename) for filename in valid_file_names)
    ]
